...
You are breaking down a PRD or feature Confluence page into specific, implementable JIRA task issues. The primary goal is to create individual JIRA issues for each identified task and attach them to the parent page for proper tracking and implementation workflow.

**Task:** Break down Confluence page $ARGUMENTS into implementation task JIRA issues

**⚠️ RATE LIMIT POLICY: Create all JIRA issues sequentially, ONE AT A TIME. Never create multiple issues in parallel to avoid hitting API rate limits.**

## Step 1: Fetch and Analyze Source Issue

**Use the Atlassian tool to gather complete information:**

1. Fetch the source confluence page details using the url from $ARGUMENTS
2. Read the full page description and all requirements
3. Identify the issue type (PRD vs Feature) from the title
...
**IMPORTANT: To avoid rate limits, create JIRA issues ONE AT A TIME sequentially. Do NOT create multiple issues in parallel.**

**For each identified task, use the Atlassian tool to create task issues with a link to the confluence doc in the description:**
...
**Set up JIRA relationships and labels:**

1. **Add labels**: `{area}` (frontend/backend/etc.), `{size}` (S/M/L)
2. **Reference in description**: Include "Documentation: {confluence_doc_link}" in the issue body
...
## Step 7: Update Parent Confluence Doc

**Use the Atlassian tool to update the parent Confluence doc:**

1. Add a task breakdown summary to the page
2. Create a checklist of all created task issues
3. Add a comment summarizing the breakdown
...
- **Parent Doc**: Confluence page URL and full title
...
- ✅ All identified tasks are captured as individual JIRA issues
...
This systematic approach ensures comprehensive task breakdown while maintaining clear traceability through JIRA's issue system, with the primary deliverable being a complete set of actionable issues.
